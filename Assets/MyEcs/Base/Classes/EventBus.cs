...
namespace Leopotam.EcsLite
{
    ...
    public class EventBus
    {
        ...
        public EventBus(int capacityEvents = 8, int capacityEventsSingleton = 8)
        {
        ...
        {
            private readonly EventBus eventsBus;
            private readonly List<Action> destructionActions;

            public DestroyEventsSystem(EventBus eventsBus, int capacity)
            {
        ...
        public ClearWorldSystem GetClearSystem()
        {
            return new ClearWorldSystem(this);
        }

        public class ClearWorldSystem : IEcsRunSystem
        {
            readonly static int bufferSize = 512;
            int[] entityBuffer;
            EventBus eBus;
            public ClearWorldSystem(EventBus bus)
            {
                eBus = bus;
                entityBuffer = new int[bufferSize];
            }
            public void Run(IEcsSystems systems)
            {
                var w = eBus.GetEventsWorld();
                int entityCount = w.GetAllEntities(ref entityBuffer);
                while (entityCount > 0)
                {
                    for (int i = 0; i < entityCount; i++)
                        w.DelEntity(entityBuffer[i]);
                    entityCount = w.GetAllEntities(ref entityBuffer);
                }
                eBus.singletonEntities.Clear();
                eBus.cachedFilters.Clear();
            }
        }
